...
import { Menu, X, Home, FolderOpen, User, FileText, Briefcase } from "lucide-react"
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="px-8 py-4 rounded-full bg-background/95 backdrop-blur-xl shadow-2xl border border-border/50 shadow-black/10">
          <div className="flex items-center space-x-8">
            {navigation.map((item) => (
              ...
              >
                {item.name}
                {pathname === item.href && (
                  <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full" />
                )}
                <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-0 h-1.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full transition-all duration-300 group-hover:w-1.5" />
              </Link>
            ))}
          </div>
        </div>
